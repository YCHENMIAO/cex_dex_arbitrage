...
from storage.dolphindb_client import DolphinDBConfig
...
if __name__ == "__main__":
    # ==================== 第零步：初始化订单簿写入器 ====================
    try:
        writer_config = DolphinDBConfig(
            host=cfg.DOLPHINDB_HOST,
            port=cfg.DOLPHINDB_PORT,
            user=cfg.DOLPHINDB_USER,
            password=cfg.DOLPHINDB_PASSWORD,
            db_path=cfg.DOLPHINDB_DB_PATH,
            table_name=cfg.DOLPHINDB_ORDERBOOK_TABLE,
        )
        globals()["orderbook_writer"] = DolphinDBWriter(
            config=writer_config,
            table_depth=cfg.DOLPHINDB_ORDERBOOK_DEPTH,
            batch_size=cfg.DOLPHINDB_BATCH_SIZE,
            flush_interval_seconds=cfg.DOLPHINDB_FLUSH_INTERVAL,
        )
        orderbook_writer = globals()["orderbook_writer"]
        orderbook_writer.start()
        Depth_Marketdata.set_writer(orderbook_writer)
        print("订单簿写入器初始化完成！")
    except Exception as e:
        orderbook_writer = None
        logging.exception("订单簿写入器启动失败，将继续运行但不写入 DolphinDB: %s", e)
        print("警告：订单簿写入器启动失败，本次运行不会写入 DolphinDB。")
    ...
    try:
        ...
        if orderbook_writer:
            orderbook_writer.stop(flush_remaining=True)
